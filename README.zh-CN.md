...
脚本会在源码更新后自动刷新本地 editable 安装，所以拉取新版后直接运行脚本即可。
...
TUI 中迁移 session 的流程是：先加载 session 列表，勾选具体要迁移的 session，再选择目标 provider，确认后执行。另有“迁移到当前 Provider”入口，会自动勾选第一行 provider 不等于当前 profile provider 的 session，同时仍允许你在写入前取消勾选某些 session。
TUI 删除 session 前会先展示所选 session 对应工作区的产出，按文档、代码、数据、图片、演示、压缩包等常见类型汇总，并识别工作区内包含 `SKILL.md` 的 skills。未全局安装的 skill 可以安装到 `~/.codex/skills`。删除 session 只会删除选中的 `.jsonl` session 文件，不会删除工作区文件。
...
在 TUI 里新增第三方 Provider 时，工具会根据 provider id 自动生成环境变量名，只要求你输入真实 API key，然后读取 `/v1/models`、为选中的模型创建 Profile，并将它设为当前使用的 Profile。真实 key 只会写入环境变量，不会写进 `config.toml`。
...
TUI 还支持删除选中的 session 文件。删除前会展示相关工作区产出和 workspace skills，并可选择是否先备份选中的 session 文件。
...
Windows 持久化设置在用户明确选择时使用 `setx`。macOS/Linux 会先设置当前进程环境变量，并提示你需要手动持久化哪个变量，但不会打印真实密钥。
